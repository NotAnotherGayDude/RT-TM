...
namespace rt_tm {
	...
	struct ct_key_hasher {
		...
		constexpr ct_key_hasher& operator++() noexcept {
			updateSeed();
			return *this;
		}

		constexpr operator bool() noexcept {
			return index < prns.size();
		}
		...
		constexpr uint64_t hashKeyCt(const data_type* value, uint64_t length) const noexcept {
	...
	template<uint64_t seedNew> struct rt_key_hasher {
		...
		RT_TM_FORCE_INLINE uint64_t hashKeyRt(const data_type* value, uint64_t length) const noexcept {
			uint64_t seed64{ seed };
			while (length >= 8) {
